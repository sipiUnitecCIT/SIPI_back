...
class StudentsService {
  async findAll() {
    ...
    //   SELECT * FROM dbo.tbl_estudiante WHERE id_persona in (${people})
    // `)
    
    const [data] = await sequelize.query(`
      SELECT * FROM dbo.tbl_estudiante
      INNER JOIN dbo.tbl_persona
      ON dbo.tbl_estudiante.id_persona = dbo.tbl_persona.id_persona
    `)
